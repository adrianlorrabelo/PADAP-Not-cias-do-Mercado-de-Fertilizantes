const { createClient } = require('@supabase/supabase-js');

const LOGO_BASE64 = '';

const CATEGORIES = ['Todas', 'Mercado', 'Ureia', 'Fosfatados', 'Potássio', 'Comércio Exterior', 'Câmbio', 'Geopolítica', 'Logística'];

function escapeHtml(str) {
  return String(str || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#39;');
}

function timeAgo(dateStr) {
  const d = new Date(dateStr);
  if (isNaN(d)) return '';
  const diffMs = Date.now() - d.getTime();
  const min = Math.floor(diffMs / 60000);
  if (min < 1) return 'agora';
  if (min < 60) return `há ${min} min`;
  const h = Math.floor(min / 60);
  if (h < 24) return `há ${h}h`;
  const days = Math.floor(h / 24);
  if (days === 1) return 'há 1 dia';
  return `há ${days} dias`;
}

function renderNewsCard(item) {
  return `
    <!-- REPETIR ESTE BLOCO PARA CADA ITEM DO RELATÓRIO -->
    <a class="news-card" href="${escapeHtml(item.link)}" target="_blank" rel="noopener noreferrer">
      <div class="card-bar"></div>
      <div class="card-body">
        <div class="tag-row">
          <span class="tag">${escapeHtml(item.category)}</span>
          <span class="time-ago">${escapeHtml(timeAgo(item.pub_date))}</span>
        </div>
        <h3 class="headline">${escapeHtml(item.title)}</h3>
        <p class="excerpt">${escapeHtml(item.excerpt)}</p>
        <div class="card-footer">
          <span class="source">${escapeHtml(item.source)}</span>
        </div>
      </div>
    </a>
    <!-- FIM DO BLOCO REPETÍVEL -->`;
}

function renderTabs(activeCategory) {
  return CATEGORIES.map(
    (cat) => `<span class="tab${cat === activeCategory ? ' active' : ''}">${escapeHtml(cat)}</span>`
  ).join('');
}

function renderReportHtml({ titulo, categoriaPrincipal, periodoReferencia, dataGeracao, horaGeracao, tituloSecao, items }) {
  const cards = items.map(renderNewsCard).join('\n');
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>PADAP · ${escapeHtml(titulo)}</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=Inter:wght@400;500;600&display=swap" rel="stylesheet">
<style>
  :root{
    --white:#FFFFFF;
    --dark-green:#123B32;
    --black:#161616;
    --accent:#2FAE0F;
    --accent-dark:#1F8A0A;
    --gray:#E8ECE9;
    --gray-text:#6B7570;
    --radius:14px;
    --max-w:1180px;
  }
  *{box-sizing:border-box;}
  html,body{margin:0;padding:0;}
  body{background:var(--white); color:var(--black); font-family:'Inter',system-ui,sans-serif; -webkit-font-smoothing:antialiased;}
  h1,h2,h3,.display{font-family:'Sora',system-ui,sans-serif;}
  a{color:inherit;}

  header{border-bottom:1px solid var(--gray);}
  .header-inner{max-width:var(--max-w); margin:0 auto; padding:18px 24px; display:flex; align-items:center; justify-content:space-between; gap:16px; flex-wrap:wrap;}
  .brand{display:flex; align-items:center; gap:12px;}
  .brand img{height:40px; width:auto; display:block;}
  .brand-text .name{font-weight:800; font-size:19px; color:var(--dark-green); letter-spacing:0.02em; line-height:1.1;}
  .brand-text .tagline{font-size:12px; color:var(--gray-text); font-weight:500; letter-spacing:0.03em; margin-top:2px;}
  .header-status{display:flex; align-items:center; gap:10px; flex-wrap:wrap;}
  .status-pill{display:flex; align-items:center; gap:7px; font-size:12.5px; font-weight:600; color:var(--dark-green); background:var(--gray); padding:6px 12px 6px 10px; border-radius:999px;}
  .status-dot{width:7px;height:7px;border-radius:50%; background:var(--accent);}
  .period-pill{font-size:12.5px; font-weight:600; color:var(--white); background:var(--dark-green); padding:6px 14px; border-radius:999px;}
  .print-btn{background:var(--dark-green); color:var(--white); border:none; border-radius:999px; padding:9px 16px; font-family:'Inter',sans-serif; font-weight:600; font-size:13px; cursor:pointer;}
  .print-btn:hover{background:var(--accent-dark);}

  .tabs{max-width:var(--max-w); margin:0 auto; padding:18px 24px 4px; display:flex; gap:8px; flex-wrap:wrap;}
  .tab{border:1px solid var(--gray); background:var(--white); color:var(--dark-green); font-family:'Inter',sans-serif; font-weight:600; font-size:13px; padding:8px 14px; border-radius:999px; white-space:nowrap;}
  .tab.active{background:var(--dark-green); border-color:var(--dark-green); color:var(--white);}

  main{max-width:var(--max-w); margin:0 auto; padding:20px 24px 80px;}
  .section-label{display:flex; align-items:baseline; justify-content:space-between; margin:10px 2px 16px;}
  .section-label h1{font-size:15px; font-weight:700; color:var(--gray-text); text-transform:uppercase; letter-spacing:0.08em; margin:0;}
  .count-text{font-size:12.5px; color:var(--gray-text);}

  .grid{display:grid; grid-template-columns:repeat(3, 1fr); gap:18px;}
  @media (max-width:920px){ .grid{grid-template-columns:repeat(2,1fr);} }
  @media (max-width:640px){ .grid{grid-template-columns:1fr;} }

  .news-card{border:1px solid var(--gray); border-radius:var(--radius); padding:0; background:var(--white); text-decoration:none; color:inherit; display:flex; flex-direction:column; overflow:hidden;}
  .card-bar{height:4px; background:var(--accent);}
  .card-body{padding:18px 18px 16px; display:flex; flex-direction:column; gap:10px; flex:1;}
  .tag-row{display:flex; align-items:center; justify-content:space-between;}
  .tag{font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; color:var(--accent-dark); background:rgba(47,174,15,0.1); padding:4px 9px; border-radius:6px;}
  .time-ago{font-size:11.5px; color:var(--gray-text); font-weight:500;}
  .headline{font-family:'Sora',sans-serif; font-weight:700; font-size:16.5px; line-height:1.35; color:var(--black); margin:0;}
  .excerpt{font-size:13px; line-height:1.55; color:var(--gray-text); margin:0;}
  .card-footer{margin-top:auto; padding-top:10px; border-top:1px solid var(--gray);}
  .source{font-size:12.5px; font-weight:600; color:var(--dark-green);}

  .state-box{grid-column:1 / -1; text-align:center; padding:70px 20px; color:var(--gray-text);}
  .state-box h2{color:var(--dark-green); font-size:18px; margin:14px 0 6px;}

  footer{border-top:1px solid var(--gray); padding:22px 24px; text-align:center; font-size:12px; color:var(--gray-text);}

  @media print{
    .print-btn{display:none;}
    .news-card{break-inside:avoid;}
  }
</style>
</head>
<body>

<header>
  <div class="header-inner">
    <div class="brand">
      <img src="data:image/png;base64,${LOGO_BASE64}" alt="PADAP">
      <div class="brand-text">
        <div class="name">PADAP</div>
        <div class="tagline">${escapeHtml(titulo)}</div>
      </div>
    </div>
    <div class="header-status">
      <div class="status-pill"><span class="status-dot"></span>Gerado em ${escapeHtml(dataGeracao)} às ${escapeHtml(horaGeracao)}</div>
      <div class="period-pill">${escapeHtml(periodoReferencia)}</div>
      <button class="print-btn" onclick="window.print()">Imprimir / Salvar PDF</button>
    </div>
  </div>
  <div class="tabs">${renderTabs(categoriaPrincipal)}</div>
</header>

<main>
  <div class="section-label">
    <h1>${escapeHtml(tituloSecao)}</h1>
    <span class="count-text">${items.length} notícia${items.length === 1 ? '' : 's'}</span>
  </div>
  <div class="grid">
    ${items.length > 0 ? cards : `
    <div class="state-box">
      <h2>Nenhuma notícia encontrada</h2>
      <p>Não há notícias salvas para esse filtro e período.</p>
    </div>`}
  </div>
</main>

<footer>
  Relatório gerado automaticamente pelo PADAP Notícias do Mercado de Fertilizantes.
</footer>

</body>
</html>
`;
}

module.exports = async function handler(req, res) {
  const category = typeof req.query.category === 'string' && req.query.category ? req.query.category : 'Todas';
  const days = Math.min(Math.max(Number(req.query.days) || 7, 1), 365);
  const customTitle = typeof req.query.title === 'string' ? req.query.title : '';

  const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_ANON_KEY);

  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString();

  let query = supabase
    .from('news_items')
    .select('title, link, excerpt, source, category, pub_date')
    .gte('pub_date', since)
    .order('pub_date', { ascending: false })
    .limit(300);

  if (category !== 'Todas') {
    query = query.eq('category', category);
  }

  const { data, error } = await query;

  if (error) {
    res.status(500).send(`Erro ao gerar relatório: ${escapeHtml(error.message)}`);
    return;
  }

  const now = new Date();
  const titulo = customTitle || (category === 'Todas' ? 'Radar de Notícias' : `Radar de Notícias — ${category}`);
  const periodoReferencia = days === 1 ? 'Últimas 24h' : days === 7 ? 'Últimos 7 dias' : `Últimos ${days} dias`;

  const html = renderReportHtml({
    titulo,
    categoriaPrincipal: category,
    periodoReferencia,
    dataGeracao: now.toLocaleDateString('pt-BR'),
    horaGeracao: now.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
    tituloSecao: 'RADAR DE NOTÍCIAS',
    items: data || [],
  });

  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.status(200).send(html);
};
