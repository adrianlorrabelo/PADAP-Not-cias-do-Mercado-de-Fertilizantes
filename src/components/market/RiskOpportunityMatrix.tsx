    ...
    <Card>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {quadrants.map((quadrant) => (
          <div key={quadrant.id} className="min-h-44 rounded-lg border border-padap-line bg-padap-field p-4">
            <Badge tone={quadrant.tone}>{quadrant.title}</Badge>
            <div className="mt-3 flex flex-wrap gap-2">
              {items.filter((item) => item.quadrant === quadrant.id).map((item) => (
                <button key={item.id} className="rounded-full border border-padap-line bg-padap-field px-3 py-1.5 text-sm text-padap-ink transition hover:border-padap-green/30" onClick={() => setSelected(item)}>{item.label}</button>
              ))}
            </div>
          </div>
        ))}
      </div>
      {selected && <div className="mt-4 rounded-lg border border-padap-cyan/20 bg-padap-cyan/[0.05] p-4 text-sm leading-6 text-padap-muted"><strong className="text-padap-ink">{selected.label}:</strong> {selected.summary}<br /><span className="text-padap-emerald">Ação: {selected.recommendedAction}</span></div>}
    </Card>
  );
}
