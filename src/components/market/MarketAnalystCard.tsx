...
export function MarketAnalystCard({ insight, onAnalysis, onBriefing }: { insight: MarketAnalystInsight; onAnalysis: () => void; onBriefing: () => void }) {
  return (
    <Card>
      <div className="mb-4 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Bot className="text-padap-cyan" size={20} /><h2 className="text-lg font-semibold text-white">Analista de Mercado</h2><Badge tone="cyan">IA</Badge></div><Badge tone="green">Alta - {insight.confidence}%</Badge></div>
