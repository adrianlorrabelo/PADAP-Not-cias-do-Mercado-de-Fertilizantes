          ...
          <article key={item.id} className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
              ...
              <div className="flex gap-2">
                <a href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-white/10 bg-white/[0.045] px-3 text-sm font-semibold text-slate-100 transition hover:border-padap-green/25"><ExternalLink size={14} />Ver noticia completa</a>
                <button type="button" className="inline-flex min-h-9 items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.045] px-3 text-sm font-semibold text-slate-100 transition hover:border-padap-green/25 hover:bg-padap-green/[0.08]" onClick={(event) => { event.preventDefault(); if (item.url) onCopy(item.url); }}><Copy size={14} />Copiar link</button>
              </div>
            </div>
          </article>
        ))}
