      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {ratios.map((ratio) => (
          <div key={ratio.id} className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
            ...
            <div className="mt-3 flex items-center justify-between"><span className={ratio.variation <= 0 ? "text-padap-mint" : "text-amber-200"}>{formatPercent(ratio.variation)}</span><Sparkline data={ratio.history} color={ratio.status === "Favorável" ? "#1dba2c" : "#f6b73c"} /></div>
