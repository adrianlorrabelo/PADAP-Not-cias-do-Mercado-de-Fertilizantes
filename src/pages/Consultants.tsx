...
export default function Consultants() {
  return (
    <div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2"><BarChartCard title="Volume cotado" data={mockConsultants.map((c, index) => ({ label: c.name.split(" ")[0], value: 420000 + index * 160000 }))} /><BarChartCard title="Produtos mais vendidos" data={[{ label: "YaraBasa", value: 42 }, { label: "YaraMila", value: 36 }, { label: "KCl", value: 29 }, { label: "MAP", value: 21 }]} color="#39d353" /></div>
      <p className="mt-4 text-sm text-slate-500">Volume fechado demonstrativo: {formatarMoedaBRL(986000)}</p>
    </div>
  );
}
